<script lang="ts">
	...
	import {yw_account, yw_network, yw_progress, yw_settings, yw_shift_key} from '../mem';
	...
	import type {SignerData} from '#/chain/cosmos-network';
	...
	import {B_ANDROID_NATIVE, B_DEVELOPMENT, B_IOS_NATIVE, B_WITHIN_IAB_NAV_IFRAME, XT_MINUTES, X_SIMULATION_GAS_MULTIPLIER} from '#/share/constants';
	import {Accounts} from '#/store/accounts';
	import {Apps} from '#/store/apps';
	import {Chains, G_CHAIN_NONE} from '#/store/chains';
	...
	const dp_network = $yw_network || yw_network.nextUpdate();
	...
	// no chain
	let g_chain_sign = g_chain;
	if(amino && !amino.chain_id) {
		g_chain_sign = G_CHAIN_NONE;
	}
	...
	async function simulate() {
		...
		for(const g_msg of a_msgs_amino) {
			// execution; can no longer simulate
	...
	async function tx_granter(): Promise<string> {
		...
		// attempt to load fee grants
		if(!k_fee_grants) {
			await dp_network;

			try {
				k_fee_grants = await FeeGrants.forAccount(g_account, $yw_network);
			}
			catch(e_load) {
				syserr({
					title: 'Fee grant unavailable',
					text: 'Failed to determine if any fee grants were available for this account:\n'+e_load.stack,
				});
				return '';
			}
		}
	...
	async function attempt_approve() {
		...
		const sa_senderc = pubkey_to_bech32(g_account.pubkey, g_chain.bech32s.acc);
		...
		if(g_amino) {
			...
			// whether it is sign-only
			let b_signonly = true;
			for(const g_msg of a_equivalent_amino_msgs) {
				if(!['query_permit', 'signature_proof'].includes(g_msg.type)) {
					b_signonly = false;
					break;
				}
			}

			if(!b_signonly && !b_no_fee) {
			...
			// attempt to sign
			try {
				let atu8_auth!: Uint8Array;
				let g_signer!: SignerData;

				if(!b_signonly) {
					({auth:atu8_auth, signer:g_signer} = await $yw_network.authInfo(g_account, {
				...
				if(!b_signonly) {
					...
					// pack amino in order to compute tx hash
					const {
						atu8_tx,
						sxb16_hash,
		...
		if(si_txn) {
			...
			CONTACTING_BACKGROUND: {
				// skip on ios
				if(B_IOS_NATIVE || B_WITHIN_IAB_NAV_IFRAME || B_ANDROID_NATIVE) break CONTACTING_BACKGROUND;
		...
		// was just for signing
			...
			// query permit
			if(g_chain.features.secretwasm && 1 === a_equivalent_amino_msgs.length && ['query_permit'].includes(a_equivalent_amino_msgs[0].type)) {
...
<Screen>
	{#await dp_account}
		<AppBanner app={g_app} chains={[g_chain_sign]} on:close={() => cancel()}>
	...
	{:then g_account}
		<AppBanner app={g_app} chains={[g_chain_sign]} account={g_account} on:close={() => cancel()}>
			<span slot="default" style="display:contents;">
	...
	<ActionsLine disabled={b_approving} back={local} cancel={!local? () => cancel(): false} confirm={[b_user_broadcast? 'Broadcast': 'Approve', approve, !b_loaded]} />
